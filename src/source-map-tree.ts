...
export default class SourceMapTree {
  ...
  traceMappings(): DecodedSourceMap {
    ...
    for (let i = 0; i < rootMappings.length; i++) {
      ...
      const traced =
        traceUneditedLine(segments, rootSources, intoSegments) ||
        traceLine(segments, rootSources, rootNames, intoSegments);
...
// An unedited line either contains no segments, or it contains only a line marker segment.
// Line markers are the default "lowres" segment generated by magic-string, and match [0, SOURCE, LINE, 0].
// For these markers, we trace directly from the referenced source instead of from this map.
