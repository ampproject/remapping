...
export default class SourceMapTree {
  ...
  traceMappings(): DecodedSourceMap {
    ...
    for (let i = 0; i < rootMappings.length; i++) {
      ...
      let tracedSegments;
      if (isEdit && isUneditedLine(segments)) {
        const segment = segments[0];
        tracedSegments = rootSources[segment[1]].traceLine(segment[2], intoSegments);
      } else {
        tracedSegments = traceLine(segments, rootSources, rootNames, intoSegments);
      }
...
function isUneditedLine(segments: SourceMapSegment[]): segments is [[0, number, number, 0]] {
